class User < ApplicationRecord
  ...
  validates_format_of :fname, with: /[-_.a-zA-Z0-9]{3,}/i, on: :create
  
  validates :lname, presence: true
  validates_format_of :lname, with: /[-_.a-zA-Z0-9]{3,}/i, on: :create
  
  validates :address, presence: true
  validates_format_of :address, with: /([0-9]+)\s([a-z\s]+)/i, on: :create
  
  validates :state, presence: true
  validates_format_of :state, with: /[a-z\s.]{3,}/i, on: :create
  
  validates :pcode, presence: true
  validates_format_of :pcode, with: /[0-9a-z]{3}\s?[0-9a-z]{2,3}/i, on: :create
  
  validates :city, presence: true
  validates_format_of :city, with: /[a-z\s.]{3,}/i, on: :create
  
  validates :country, presence: true
  validates_format_of :country, with: /[a-z\s.]{3,}/i, on: :create
